...
const User = (props) => {
  ...
  const pageChange = (newPage) => {
    currentPage !== newPage &&
      history.push(
        `/users/${props.fileName.match.params.year}/${props.fileName.match.params.type}/${props.fileName.match.params.subject}`
      );
  };
              ...
              scopedSlots={{
                filePath: (item) => (
                  <td>
                    <a
                      href="#/"
                      onClick={() => downloadFile(item._id, item.filePath)}
                    >
                      Download
                    </a>{" "}
                  </td>
